...
const editableWaypointSegments = computed(() => {
    if (!editableWaypoints.value || editableWaypoints.value.length < 2) {
        return [];
    }
    return calculateWaypointSegments(
...
function getSegmentForWaypoint(waypointId: string): WaypointSegment | null {
    return getSegmentAfterWaypoint(waypointId, editableWaypointSegments.value);
}
...
function handleMapLineClick(coords: { lat: number; lng: number }) {
    ...
    if (distanceAtClick !== null) {
        ...
        if (
            newWaypointBeingCreated.value &&
            selectedWaypointForEdit.value.id ===
...
function updateWaypointFromDistanceInput() {
    ...
    if (newPosition) {
        ...
        if (newWaypointBeingCreated.value) {
            ...
            newWaypointBeingCreated.value.order =
                Math.floor(newDistanceInMeters);
            newWaypointBeingCreated.value.lat = newPosition.lat;
            newWaypointBeingCreated.value.lng = newPosition.lng;
            newWaypointBeingCreated.value.elevation = newElevation;
                    ...
                    <div v-else class="h-full flex flex-col overflow-hidden">
                        ...
                        <div class="flex-1 overflow-y-auto">
                            ...
                            >
                                ...
                                <div
                                    v-if="index < editableWaypoints.length - 1"
