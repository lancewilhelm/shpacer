...
<template>
    <div ref="popupRef" class="relative">
        <button
            class="flex items-center gap-2 px-2 py-1 md:px-3 md:py-2 border border-(--main-color) text-(--main-color) rounded-sm hover:bg-(--main-color) hover:text-(--bg-color) transition-colors text-xs md:text-sm focus-outline"
        ...
        <div
            v-if="popupVisible"
            class="absolute top-full mb-2 mt-2 left-0 md:left-auto md:right-0 bg-(--bg-color) border border-(--sub-color) rounded-lg shadow-lg min-w-max max-w-[90vw] max-h-82 z-50 overflow-y-auto"
        >
            <div class="p-2">
                <div
