  ...
  () => {
    ...
    if (props.existingPlan) {
      ...
      if (p.pace != null) {
        ...
        formData.value.paceMinutes = Math.floor(paceRounded / 60).toString();
      ...
      if (
        ...
        formData.value.targetMinutes = minutes.toString().padStart(2, "0");
        formData.value.targetSeconds = seconds.toString().padStart(2, "0");
      } else {
        formData.value.targetHours = "";
        formData.value.targetMinutes = "";
        formData.value.targetSeconds = "";
      ...
      if (formData.value.pacingStrategy === "linear") {
        const lp =
          typeof p.pacingLinearPercent === "number" ? p.pacingLinearPercent : 0;
      ...
      formData.value.useGradeAdjustment =
        typeof p.useGradeAdjustment === "boolean" ? p.useGradeAdjustment : true;
    } else {
      resetForm();
    }
    error.value = "";
...
function validatePaceFields(): {
  ...
  if (isBlank(mRaw) || isBlank(sRaw)) {
    return {
      ok: false,
      message: "Please fill both pace minutes and seconds or leave both blank.",
      seconds: null,
    };
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (props.existingPlan) {
      ...
      const response = await fetch(`/api/courses/${props.courseId}/plans`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
    ...
    <div class="p-2 max-w-md w-full h-[720px] flex flex-col">
      ...
      <div class="mb-4">
        ...
        <p class="mt-2 text-xs text-(--sub-color)">
          Use Next and Back or click the steps above to navigate. You can Save
          from any step once required fields are complete.
        </p>
      </div>
      ...
      >
        ...
        <div v-show="activeStep === 2">
          ...
          <div class="mt-3">
            ...
            <p class="text-xs text-(--sub-color) mt-1">
              Segment paces and arrival times will account for elevation
              changes.
            </p>
          </div>
        </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            >
              <input
                ...
                :disabled="isSubmitting"
              />
              <span>Flat pacing (uniform from start to finish)</span>
            </label>
            <label
              class="flex items-center gap-2 cursor-pointer text-(--main-color) shrink-0"
            >
              <input
                ...
                :disabled="isSubmitting"
              />
              <span>Linear pacing (gradual change across the course)</span>
            </label>
          </div>

          <div v-if="formData.pacingStrategy === 'linear'" class="mt-2">
            <label class="block text-sm font-medium text-(--main-color) mb-1">
            ...
            <p class="text-xs text-(--sub-color) mt-1">
              Positive values start slightly faster and finish slightly slower.
              For example, 10% varies the effort factor from 0.95 at the start
              to 1.05 at the finish. Negative values produce negative splits.
            </p>
          </div>
        </div>

        <!-- Pace Inputs -->
        <div v-if="formData.paceMode !== 'time'" v-show="activeStep === 2">
          <label
            class="block text-sm font-medium text-(--main-color) mb-1"
            :for="'pace-minutes'"
          >
          ...
          <p class="text-xs text-(--sub-color) mb-2">
            <span v-if="formData.paceMode === 'normalized'">
              Enter the pace you can comfortably sustain on a normalized
              (flat-equivalent) course. Segment paces adjust for grade.
            </span>
            <span v-else>
              Desired overall average pace; segment paces adjust for grade.
            </span>
          </p>
          <div class="flex items-center gap-2">
            <!-- Pace Minutes -->
            <div class="flex flex-col items-center">
              ...
              <span class="text-xs text-(--sub-color) mt-1">min</span>
            </div>
            <div class="-translate-y-2.5 text-(--main-color)">:</div>
            <!-- Pace Seconds -->
            <div class="flex flex-col items-center">
              <button
                type="button"
              ...
              <span class="text-xs text-(--sub-color) mt-1">sec</span>
            </div>
            <!-- Pace Unit -->
            <div class="flex flex-col justify-center -translate-y-2.5">
              <select
          ...
          <p class="text-xs text-(--sub-color) mt-1">
            Leave both fields blank for no explicit pace (optional). Seconds
            must be &lt; 60.
          </p>
        </div>
        ...
        <div v-else v-show="activeStep === 2">
          ...
          <p class="text-xs text-(--sub-color) mb-2">
            Set the total time you’re targeting. We'll derive an average base
            pace.
          </p>
          <div class="flex items-center gap-2">
            <!-- Hours -->
            <div class="flex flex-col items-center">
              ...
              <span class="text-xs text-(--sub-color) mt-1">hr</span>
            </div>
            <div class="-translate-y-2.5 text-(--main-color)">:</div>
            <!-- Minutes -->
            <div class="flex flex-col items-center">
          ...
          <p class="text-xs text-(--sub-color) mt-1">
            Provide all three fields. Minutes & seconds must be &lt; 60; cannot
            be all zeros.
          </p>
          <p v-if="courseTotalDistance" class="text-xs text-(--sub-color) mt-1">
            Course distance:
            {{
              (
                courseTotalDistance /
                (formData.paceUnit === "min_per_km" ? 1000 : 1609.344)
              ).toFixed(2)
            }}
        ...
        <div v-show="activeStep === 2" class="mt-3">
          ...
          <p class="text-xs text-(--sub-color) mt-1">
            When enabled, your target time/pace is treated as total elapsed time
            (including planned waypoint stops). We'll subtract planned waypoint
            stoppage time when deriving moving paces and arrival times.
          </p>
        </div>
        ...
        <div v-show="activeStep === 1">
          ...
          <p class="text-xs text-(--sub-color) mt-1">
            Optional. Applies only to intermediate waypoints. Leave both blank
            for none.
          </p>
        </div>
      ...
      <div class="grid grid-cols-4 gap-3 pt-4 items-stretch">
        ...
        >
          ...
          >
            <Icon name="svg-spinners:6-dots-scale" class="scale-75" />
            Saving...
          </span>
