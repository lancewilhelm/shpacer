...
async function handleFiles(files: File[]) {
    ...
    try {
        ...
        const processedFileData: ProcessedFile = {
            name: validFile.name,
            originalContent,
            fileType: validFile.name.toLowerCase().endsWith(".gpx")
                ? "gpx"
                : "tcx",
            geoJson,
        };
...
<template>
    <div class="w-full">
        ...
        >
            ...
            <div class="flex flex-col items-center gap-4">
                ...
                <div v-else class="flex items-center gap-4">
                    ...
                    />
                    <p class="text-sm text-(--main-color)">
                        Processing files...
                    </p>
                </div>
        ...
        <div v-if="uploadedFile" class="mt-4">
            <h3 class="text-sm font-medium text-(--main-color) mb-2">
                Uploaded File:
            </h3>
            <div
                class="flex items-center justify-between p-2 bg-(--sub-alt-color) rounded-lg border border-(--sub-color)"
            >
        ...
        <ModalWindow :open="showGeoJsonModal" @close="closeGeoJsonModal">
            <div class="max-w-4xl w-full">
                ...
                >
                    <pre
                        class="text-sm text-(--main-color) whitespace-pre-wrap"
                        >{{
                            uploadedFile
                                ? JSON.stringify(uploadedFile.geoJson, null, 2)
                                : ""
                        }}</pre
                    >
                </div>
