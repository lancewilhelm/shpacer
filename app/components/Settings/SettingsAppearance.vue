...
const favoriteThemes = computed(() =>
    JSON.parse(JSON.stringify(themesList))
        .filter((theme: Theme) => {
            ...
            return userSettingsStore.settings.favoriteThemes.includes(
                theme.name,
            );
        })
...
const chartAreaGradientEnabled = computed(() => {
    return Boolean(
...
<template>
    <div class="w-full">
        ...
        <SettingsGroup
            ...
            description="customize chart style"
        >
            <SettingsSubGroup title="chart style" icon="lucide:paintbrush">
                <SettingsToggleItem
                    :value="chartAreaGradientEnabled"
