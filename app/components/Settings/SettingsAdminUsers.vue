...
const fetchUsers = async () => {
    ...
    if (error) {
        logger.error(`Error fetching users: ${error?.message ?? "unknown"}`);
        return;
    }
...
async function resetUserPassword(userId: string, email?: string) {
    const confirmed = confirm(
        `Are you sure you want to reset the password for ${email || "this user"} to "password"?`,
    );
    if (!confirmed) return;
...
const editUserOriginalEmail = ref("");
const editUserEmailInput = ref<HTMLInputElement | null>(null);
const editUserRole = ref<"admin" | "user">("user");

watch(editUserModalVisible, (open) => {
    if (open) {
        const target = users.value.find((u) => u.id === editUserId.value);
        editUserRole.value = target?.role === "admin" ? "admin" : "user";
    }
});

async function editUser() {
    if (!editUserEmail.value) {
    ...
    try {
        ...
        if (editUserEmail.value !== editUserOriginalEmail.value) {
        ...
        const targetUser = users.value.find((u) => u.id === editUserId.value);
        const currentRole = targetUser?.role === "admin" ? "admin" : "user";
        if (editUserRole.value !== currentRole) {
...
function canEditUser(user: UserWithRole) {
    ...
    if (user.role === "admin" && currentUser.value.role !== "owner")
        return false; // Can't edit admin if not owner
    ...
    <div class="w-full">
                ...
                <table class="min-w-full">
                    <thead class="bg-(--sub-color) text-(--main-color)">
                        <tr>
                            <th
                                scope="col"
                                class="px-6 py-1 text-left font-medium"
                            >
                                email
                            </th>
                            <th
                            ...
                            <th
                                scope="col"
                                class="px-6 py-1 text-left font-medium w-[150px]"
                            >
                                actions
                            </th>
                        </tr>
                    ...
                    <tbody
                        class="divide-y divide-(--sub-color) text-(--text-color)"
                    >
            ...
            <SettingsToggleItem
                ...
                @toggle="
                    globalSettingsStore.updateSettings({
                        allowRegistration:
                            !globalSettingsStore.settings.allowRegistration,
                    })
                "
            />
        </SettingsGroup>
        ...
        >
            <div class="flex flex-col items-center justify-center gap-2">
                <div class="text-(--main-color) text-lg text-center">
                    Edit User
                </div>
                <div class="text-(--text-color) text-sm text-center">
                ...
                <div class="flex gap-2">
                    ...
                    <button
                        class="flex items-center gap-2 mt-2 bg-(--error-color) text-(--bg-color) p-2 rounded-lg px-4 cursor-pointer"
                        @click="
                            () => resetUserPassword(editUserId, editUserEmail)
                        "
                    >
                        reset password
                    </button>
                    <button
                        class="flex items-center gap-2 mt-2 bg-(--sub-alt-color) text-(--text-color) p-2 rounded-lg px-4 cursor-pointer"
                        @click="editUserModalVisible = false"
                    >
                        cancel
                    </button>
                </div>
        ...
        >
            <div class="flex flex-col items-center justify-center gap-2">
                <div class="text-(--text-color) text-lg text-center">
                    Are you sure you want to delete {{ deleteUserEmail }}? This
