  ...
  <div class="flex flex-col w-full h-full overflow-hidden">
    ...
    <div class="w-full h-full p-4 flex flex-col gap-4 overflow-auto">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-(--main-color)">Courses</h1>
      ...
      <div v-else-if="error" class="flex items-center justify-center py-12">
        <div class="text-center">
          <Icon name="heroicons:exclamation-triangle" class="h-12 w-12 text-(--error-color) mx-auto mb-4" />
      ...
      <div v-else-if="courses.length === 0" class="flex items-center justify-center py-12">
        <div class="text-center">
          <Icon name="heroicons:map" class="h-16 w-16 text-(--sub-color) mx-auto mb-4 scale-300" />
          <h3 class="text-xl font-semibold text-(--main-color) mb-2">No courses yet</h3>
          <p class="text-(--sub-color) mb-4">Create your first course by uploading a GPX or TCX file</p>
          <NuxtLink
            to="/courses/new"
        ...
        >
          <div class="flex h-full gap-4">
            ...
            <div class="flex flex-col flex-1 min-w-0">
              ...
              <!-- Footer -->
              <div class="flex items-center justify-between text-xs text-(--sub-color) mt-auto">
                <span class="truncate">{{ course.originalFileName }}</span>
                <span class="flex-shrink-0 ml-2">{{ formatDate(course.createdAt) }}</span>
              </div>
            </div>
          </div>
        </div>
